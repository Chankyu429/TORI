...
class Program
{
    ...
    {
        Sword, Axe, Hammer, Bow, Staff, Dagger,
    }
    ...
    private static void WritBinaryFile(string path)
    {
        ...
        string filePath = Path.Combine(path, "weaponData.bin");
        ...
        {
            BinaryWriter wrier = new BinaryWriter(fileStream);
            {
                wrier.Write((int)data.Type);
                wrier.Write(data.Power);
                wrier.Write(data.Speed);
                wrier.Write(data.Price);
            }
            wrier.Close();
        }
        fileStream.Close();
...
```
> ``ReadInt32``, ``wrier``, ``.bin``, ``fileStream.Close``, ``BinaryWriter(fileStream)``을 새롭게 배움!
>
> 프로퍼티를 이제는 그냥 사용하기 시작함.
>
> ``ReadSingle``도 배움!
